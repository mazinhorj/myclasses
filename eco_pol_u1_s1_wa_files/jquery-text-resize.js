  ...
  $.textResize = function(selector, settings) {
    ...
    if (config.containerInsert !== false) {
      ...
      if (config.optionInterface === 'link') {
        ...
        htmlContent = '<button class="btn btn-primary mr-2 mt-2" type="button" id="btn_print" title="Imprimir" data-toggle="tooltip" data-placement="bottom"><i class="fa fa-print"></i></button>' + '<input type="button" value="' + config.reduceText + '" class="btn btn-primary mr-2 mt-2 ' + config.classReduce.replace('.', '') + '" title="Diminuir fonte" data-toggle="tooltip" data-placement="bottom"  />' +
                        '<input type="button" value="' + config.resetText + '" class="btn btn-primary mr-2 mt-2 ' + config.classReset.replace('.', '') + '" title="Fonte padrão" data-toggle="tooltip" data-placement="bottom" />' +
                        '<input type="button" value="' +  config.enlargeText + '" class="btn btn-primary mr-2 mt-2 ' +  config.classEnlarge.replace('.', '') + '" title="Aumentar fonte" data-toggle="tooltip" data-placement="bottom" />';
      }
...
document.getElementById('btn_print').onclick = function() {
  $("a, p, h1, h2, h3, h4, h5, h6, img, figcaption, button, footer, span, div, ul, ol, li, code, dl, dt, pre, blockquote, dd, th, tbody, thead, tfoot, col table").removeAttr("data-aos");
  window.print();
};
    ...
    $containerInsert.on('click', config.classReduce, function() {
      if (config.clicks > userClicksReduce) {
        ...
        ++userClicksReduce;
          console.log(userClicksReduce + " diminuir")
        // Decremento no contador ENLARGE
        if (userClicksEnlarge > 0){
          --userClicksEnlarge;
        }
          
        if(userClicksReduce == 3)  {
            $( ".aumentar" ).removeClass( "disabled" );
            $( ".diminuir" ).addClass( "disabled" );
        }else{
            $( ".diminuir" ).removeClass( "disabled" );
        }  
    ...
    $containerInsert.on('click', config.classEnlarge, function() {
      if (config.clicks > userClicksEnlarge) {
        ...
        console.log(userClicksEnlarge + 'aumentando');
        if(userClicksEnlarge == 3)  {
            $( ".aumentar" ).addClass( "disabled" );
            $( ".diminuir" ).removeClass( "disabled" );
        }else{
            $( ".aumentar" ).removeClass( "disabled" );
        }
